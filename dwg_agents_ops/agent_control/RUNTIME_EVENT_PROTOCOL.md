...
本协议只定义“运行守护事件”本身，不覆盖本地监督对象的全部职责。

在真实打印任务里，本地监督对象除消费本协议中的运行事件外，还应进一步督导：

1. 每完成一个 DWG 后是否已执行关图与 `cad_zt_oneb()` 归一
2. 执行者是否已明确受控入口与恢复入口
3. 执行者是否已掌握当前任务必须依赖的系统知识和打印规则

这些属于“更广义的监督职责”，不应再被误解为仅仅是事件转发。
