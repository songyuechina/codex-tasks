...
# -*- coding: utf-8 -*-
"""
单函数测试：cad_geometry_draw.draw_lwpolyline_wcs
...
def test_draw_lwpolyline_wcs():
    tmp_path = str(Path(current) / "tests" / "_tmp" / "test_draw_lwpolyline.dwg")
    ...
    try:
        ...
        pts = [(0.0, 0.0, 0.0), (100.0, 0.0, 0.0), (100.0, 50.0, 0.0), (0.0, 50.0, 0.0)]
        obj = draw.draw_lwpolyline_wcs(pts, closed=True, docname=None)
        wait_quiescent(min_quiet=0.3, timeout=10.0)

        after = ms.Count
        assert obj is not None, "draw_lwpolyline_wcs 返回 None"
        assert after == before + 1, f"对象数量未增加: before={before}, after={after}"
        assert "Polyline" in getattr(obj, "ObjectName", ""), f"对象类型异常: {getattr(obj, 'ObjectName', '')}"
        assert bool(getattr(obj, "Closed", False)) is True, "多段线未闭合"

        print("[PASS] draw_lwpolyline_wcs")
        return True
    finally:
        try:
            CAD_core.save_file()
