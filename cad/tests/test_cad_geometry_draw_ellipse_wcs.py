...
# -*- coding: utf-8 -*-
"""
Single-function test: cad_geometry_draw.draw_ellipse_wcs
...
def test_draw_ellipse_wcs():
    tmp_path = str(Path(current) / 'tests' / '_tmp' / 'test_draw_ellipse_wcs.dwg')
    _launch_cad()
    ...
    try:
        ...
        obj = draw.draw_ellipse_wcs(
            center=(100.0, 100.0, 0.0),
            major_axis_vec=(60.0, 0.0, 0.0),
            ratio=0.5,
            start_angle=0.0,
        ...
        after = ms.Count
        assert obj is not None, 'draw_ellipse_wcs returned None'
        assert after == before + 1, f'entity count mismatch before={before}, after={after}'
        assert 'Ellipse' in getattr(obj, 'ObjectName', ''), f'unexpected object type: {getattr(obj, "ObjectName", "")}'

        print('[PASS] draw_ellipse_wcs')
        return True
    finally:
