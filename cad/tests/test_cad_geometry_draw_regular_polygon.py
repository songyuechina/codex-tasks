...
# -*- coding: utf-8 -*-
"""
Single-function test: cad_geometry_draw.draw_regular_polygon_lwpolyline
...
def test_draw_regular_polygon_lwpolyline():
    tmp_path = str(Path(current) / 'tests' / '_tmp' / 'test_draw_regular_polygon.dwg')
    _launch_cad()
    ...
    try:
        ...
        obj = draw.draw_regular_polygon_lwpolyline(center=(100.0, 100.0, 0.0), radius=50.0, sides=6, docname=None)
        wait_quiescent(min_quiet=0.3, timeout=10.0)

        after = ms.Count
        assert obj is not None, 'draw_regular_polygon_lwpolyline returned None'
        ...
        assert bool(getattr(obj, 'Closed', False)) is True, 'polygon polyline should be closed'

        print('[PASS] draw_regular_polygon_lwpolyline')
        return True
    finally:
...
if __name__ == '__main__':
    ok = test_draw_regular_polygon_lwpolyline()
    if not ok:
        raise SystemExit(1)
