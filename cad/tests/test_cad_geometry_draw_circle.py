...
# -*- coding: utf-8 -*-
"""
Single-function test: cad_geometry_draw.draw_circle_wcs
...
def test_draw_circle_wcs():
    tmp_path = str(Path(current) / 'tests' / '_tmp' / 'test_draw_circle.dwg')
    _launch_cad()
    ...
    try:
        ...
        obj = draw.draw_circle_wcs(center=(100.0, 200.0, 0.0), radius=75.0, docname=None)
        wait_quiescent(min_quiet=0.3, timeout=10.0)

        after = ms.Count
        assert obj is not None, 'draw_circle_wcs returned None'
        assert after == before + 1, f'entity count mismatch before={before}, after={after}'
        assert 'Circle' in getattr(obj, 'ObjectName', ''), f'unexpected object type: {getattr(obj, "ObjectName", "")}'
        assert abs(float(getattr(obj, 'Radius', 0.0)) - 75.0) < 1e-6, f'unexpected radius: {getattr(obj, "Radius", None)}'

        print('[PASS] draw_circle_wcs')
        return True
    finally:
