...
TEMPLATE = SHARED_DIR / "close_demo_b.dwg"
...
def run_demo(target: Path) -> None:
    ...
    try:
        ...
        print(f"[close_file_no_save_demo] 打开: {work_file}")
        ok = cfo_open_file(str(work_file))
        if not ok:
            raise RuntimeError("open_file 返回 False")
        li()
        save_file()
        cfo_close_file("no_save")
        write_log(f"close_file_no_save_demo → {work_file}")
    finally:
        cad_zt_oneb()
        print("[close_file_no_save_demo] 已回到 cad_zt_oneb 状态")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测试 close_file(no_save)")
    parser.add_argument("output", nargs="?", default="close_file_no_save_demo.dwg",
                        help="工作 DWG 文件名")
