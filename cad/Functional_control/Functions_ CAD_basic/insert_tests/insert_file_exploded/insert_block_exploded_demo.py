...
from CAD_file_operations import new_file, save_file, close_file, insert_file_exploded
...
DEFAULT_TARGET = FUNCTION_DIR / "block_insert_exploded.dwg"
...
def parse_args():
    ...
    if len(args) not in (2, 6):
        print("用法: python insert_block_exploded_demo.py <source_dwg> <target_dwg> "
              "[x y z scale]")
        print("示例: python insert_block_exploded_demo.py source.dwg block_insert_exploded.dwg "
              "8000 0 0 1.0")
        sys.exit(1)
    ...
    if len(args) == 2:
        ...
        params = tuple(map(float, args[2:6]))
    return src, tgt, params


def main():
    source, target, (x, y, z, scale) = parse_args()
    ensure_default_source(source)
    new_file(str(target), close_after=False)
    li()
    insert_file_exploded(str(source), x=x, y=y, z=z, scale=scale)
    save_file()
