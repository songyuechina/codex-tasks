...
from CAD_file_operations import new_file, save_file, close_file, insert_region_from_file
...
DEFAULT_EXPLODE = FUNCTION_DIR / "fromfile_explode.dwg"
DEFAULT_KEEP = FUNCTION_DIR / "fromfile_keep.dwg"
...
def run_case(source: Path, target: Path, region, target_point, explode_flag: bool):
    ...
    li()
    insert_region_from_file(str(source), *region, *target_point, explode=explode_flag)
    save_file()
...
def parse_args():
    args = sys.argv[1:]
    if len(args) != 9:
        print("用法: python insert_from_explode.py <source_dwg> <explode_target_dwg> <keep_target_dwg> "
              "<x1> <y1> <x2> <y2> <x3> <y3>")
        print("示例: python insert_from_explode.py source.dwg fromfile_explode.dwg fromfile_keep.dwg "
              "0 0 1500 1500 6000 0")
        sys.exit(1)
