    ...
    while True:
        try:
            ...
            try:
                ...
                doc_count = -1 
                try: doc_count = app.Documents.Count
                except: pass 

                if doc_count == 0:
                    try:
                        sys_logger.info("[licad] 无文档，正在新建...")
                ...
                if time.time() - t0 > max_wait:
                    raise RuntimeError("AutoCAD 忙碌或无响应")
...
def get_acad_doc(max_wait=15.0): # 增加默认等待时间，天正启动很慢
    """
    [底层原语] 获取/启动 AutoCAD 应用和文档
    【V2.0 修复双进程问题】：当检测到 CAD 进程存在但未就绪时，强制等待而不是新建。
    """
    import win32com.client
    import time
    ...
    while True:
        ...
        try:
            ...
            if _is_acad_process_running():
                ...
                #    我们要做的就是：等！死等！
                if time.time() - t0 > max_wait:
                    raise RuntimeError(f"检测到 CAD 进程存在，但在 {max_wait}秒内无法建立 COM 连接 (可能是僵尸进程或卡死)")
