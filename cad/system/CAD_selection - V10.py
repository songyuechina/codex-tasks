...
__all__ = [
    ...
    'select_line', 'select_circle', 'select_ellipse', 'select_polyline', 'select_spline',
...
try:
    ...
    print("❌ [严重错误] CAD_selection 无法导入 licad，请检查路径配置！")
    C = None

# =================================================================
# 2. 核心工具与类型转换 (Core Utils)
# =================================================================
...
def _maybe_cast(ent):
    """
    【CORE-002】智能类型转换 (终极混合模式)
    策略：
    1. 查表 CastTo (最优先，获取全功能属性，如 Coordinates, Radius)
    2. Dynamic Dispatch (次优先，对于表中没有的对象，强制转为动态对象)
    3. 原始对象 (最后兜底，主要用于天正对象)
    """
    if ent is None: return None
    
    # 0. 性能快车道 (防止重复转换)
    # 检查是否已经是具体的子类接口 (不仅仅是 IAcadEntity)
    # 这里简单判断：如果有 CLSID 且不是通用的 IAcadEntity，就认为是转换好的
    if hasattr(ent, 'CLSID'):
        # 稍微激进一点：只要是强类型，先假设它是好的。
        # 如果你发现某些强类型依然缺属性，可以去掉这个 if，强制重新 Cast
        pass 

    try:
        # 1. 获取对象类型名 (如 "AcDbPolyline")
        # 必须先 EnsureDispatch 才能安全访问 ObjectName，否则有时会报 AttributeError
        temp_ent = win32.gencache.EnsureDispatch(ent)
        obj_name = getattr(temp_ent, "ObjectName", "")

        # 2. 【核心】查表强转 (解决 IAcadEntity 缺失子类属性的核心方案)
        if obj_name in _CAST_MAP:
            target_interface = _CAST_MAP[obj_name]
            return CastTo(temp_ent, target_interface)
        
        # 3. 【兜底】表中没有的标准对象 (未知类型的 CAD 实体)
        # 如果不是天正对象(TDb)，尝试用 Dynamic 包装
        # 这样即使没有 CastTo，也能像 VBA 的 Object 类型一样直接调用属性
        ...
        # 4. 天正对象 (TDb*) 或 转换失败的对象
        # 保持原样，以便通过 Invoke / DISPID 访问
        return temp_ent
...
def get_last_n_objects(n=1, autocast=True):
    ...
    mp = C.mp
    try:
        count = com_retry(lambda: mp.Count)
        if count == 0: return []
        start = max(0, count - n)
        objs = []
...
def select_tuceng(layer_names, max_retries=5, delay=0.5, autocast=True):
    ...
    for k in range(1, max_retries+1):
        try:
            return ss_select("all", filter_types=[8], filter_data=[layers[0] if len(layers)==1 else layers], autocast=autocast)
        except Exception:
            time.sleep(delay)
    return []
...
def select_kuai(max_retries=5, autocast=True):
...
def select_all_texts_mixed(target_space="Model"):
    ...
    final = []
    for obj in raw:
        if getattr(obj, "ObjectName", "") in target_names:
            final.append(obj)
    return final

def select_pub_text_entities():
    """【SEL-TYPE-012】PUB_TEXT 图层天正文字"""
...
def yin_to_xian_safe(LB, wait_s=0.1):
    """【VIS-002】隐性->显性 (LISP sssetfirst法，推荐)"""
    if not LB: return
    doc = C.doc
    try:
        ...
        print(f"LISP Select Error: {e}")

def xian_to_yin_pickfirst(clear_grips=True, autocast=True):
    """【VIS-003】获取 Pickfirst 选择集"""
    try:
        doc = C.doc
        ss = doc.PickfirstSelectionSet
        if ss.Count == 0: return []
        items = [ss.Item(i) for i in range(ss.Count)]
...
def select_entities_in_window(x1, y1, x2, y2, ty=1.0, select_mode="_W"):
    ...
    return [ent for ent in doc.PickfirstSelectionSet]
...
def unhide_all(space=None, filter_names=None, highlight=False):
    """【VIS-009】显示隐藏对象"""
    target = space if space else C.mp
    revealed = []
...
def _resolve_attr_case_insensitive(obj, attr_name):
    ...
    if cache_key in _ATTR_CASE_CACHE:
        return _ATTR_CASE_CACHE[cache_key]

    # 4. 暴力遍历 (Brute force fallback)
    try:
        all_attrs = dir(obj) 
...
def set_attr(obj, name, value):
    ...
    try:
        ...
        real_name = _resolve_attr_case_insensitive(real_obj, name)
        if real_name:
            setattr(real_obj, real_name, value)
            return True
    except: pass
    return False
