...
# —— 单实例锁文件 ——
LOCK_FILE = Path(__file__).parent / "cad_dialog_killer.lock"
...
def is_already_running():
    ...
    try:
        ...
        # 检查该PID是否存在且是cad_dialog_killer进程
        if psutil.pid_exists(pid):
            proc = psutil.Process(pid)
            if "python" in proc.name().lower() and "cad_dialog_killer" in " ".join(proc.cmdline()):
                return True
    except:
        pass
...
def main():
    ...
    if is_already_running():
        ...
        print("[警告] cad_dialog_killer 已在运行，无需重复启动")
        sys.exit(0)
    ...
    try:
        ...
        remove_lock()
        logger.info("cad_dialog_killer 已停止")
