...
# CAD_System_Queue.py
# 版本 V3.1
# 更新说明: 针对图纸空间(Layout)流程深度适配，完善了Repair和Step5中的布局参数传递。

# ==============================================================================
# 1. IDLE 引导程序 (后台引擎代码 - 保持不变，但建议重新写入以防万一)
# ==============================================================================
...
class MasterRunner:
    def __init__(self, root, user_name, service_path):
        ...
        self.root.title(f"CAD 基础服务 (IDLE版) V3.1 - {user_name}")
...
# ---------------------------------------------------------------------
        # Step 3: 插入图签 (修正：图纸空间模式下，Step 3 仅插入，不修复)
        # ---------------------------------------------------------------------
...
if not GLOBAL_CTX.get('dy'): 
    ...
    if style == "模型空间样式":
        # 模型空间：插入 + 提示检查 (通常不需要 repair_sp_insert)
        cb.insert_and_scale_labels_area_power(dy, layername="dy_quyu", delpan=0, debug=True)
        show_top_alert("完成", "图签插入完成。\\n请检查块是否成功剥离。", confirm=False)
        
    elif style == "图纸空间样式":
        # 【修正点】图纸空间：Step 3 只管插入，绝对不执行修复
        cb.insert_and_scale_labels_paper_space(dy, layername="dy_quyu", layout_name="{layout}", delpan=0, debug=True)
        # 提示用户下一步操作
        show_top_alert("交互", "图签已插入到布局 '{layout}'。\\n\\n⚠️ 注意：线条尚未修复。\\n请检查位置无误后，点击【Step 3+: 图形修复】。", confirm=False)
        
    else: 
        # 混合模式
        cb.insert_and_scale_labels_area_power(dy, layername="dy_quyu", delpan=0, debug=True)
        # 混合模式通常需要手动处理，这里暂时保持原样或根据需求调整
        cb.repair_sp_insert(target_layout_name="{layout}")
...
'''
        ...
        # ---------------------------------------------------------------------
        # Step 3+: 独立修复 (修正：增加 target_layout_name 参数)
        # ---------------------------------------------------------------------
...
if style == "模型空间样式":
    ...
    # 【确认点】图纸空间：cha_Y = 20 (布局单位通常是毫米，20mm作为阈值是合理的)
    # 无需 show_top_alert 提示剪切，因为 Step 3 直接插入到了布局
    print(f"正在扫描布局: {layout} ...")
    ...
    # 混合模式
    if show_top_alert("交互", "请确保已将图纸空间部分剪切到布局 '{layout}'。\\n点击确定开始重建。", confirm=True):
        print("处理布局部分...")
        ctq_p = cb.rebuild_print_area_title_mapping_paper(layout_target="{layout}", core_layer="dy_quyu_H", final_poly_layer="tuqian_neibu_pl", cha_Y=20, mute_logs=False)
        GLOBAL_CTX['ctq_p'] = ctq_p
        save_result(ctq_p, "_{layout}")
...
if not target_map: 
    ...
    style = "{ctx['style']}"
    if style == "图纸空间样式":
        # 【修正】布局模式：传入 layout_target
        cb.process_drawing_names_and_fill_titleblocks(target_map, layout_target="{layout}", num1=14, num2=3, layername="DIM_SYMB", start_index=1, prefix=None)
    else:
        # 【修正】模型模式：layout_target 不传 (保持默认 None 或旧接口兼容)
...
if not target_map: print("❌ 内存无映射数据")
    ...
    if style == "图纸空间样式":
        # 【修正】布局模式
        cb.build_full_print_dict_and_export_excel(target_map, layout_target="{layout}", template_path=None, output_path=None, start_index=1)
    else:
        # 【修正】模型模式
...
if not target_map: print("❌ 内存无映射数据")
    ...
    if style == "图纸空间样式":
        # 【修正】布局模式
        cb.read_excel_and_update_cad_titleblocks(target_map, layout_target="{layout}", excel_path=None, start_index=1)
    else:
        # 【修正】模型模式
