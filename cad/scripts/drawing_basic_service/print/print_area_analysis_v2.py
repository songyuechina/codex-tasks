...
# =============================================================================
# Layout-aware print-area analysis (Model treated as a special layout)
# =============================================================================

def get_print_areas_by_layout(
    *,
    include_model: bool = True,
    exclude_model_tab: bool = True,
    min_side: float = 0.0,
) -> Dict[str, List[Any]]:
    """
    以“布局名”为主键返回打印区域多段线列表。

    设计约定：
      - 模型空间按布局名 "Model" 处理（与 AutoCAD 的布局 Tab 一致）。
      - 对每个布局：先切换到该布局，再在其 PaperSpace 中分析矩形多段线并执行
        去重 -> 极大矩形 -> 删除伪外框 -> 重新极大矩形，得到最终打印区域。

    返回：
      {
        "Model": [poly1, poly2, ...],          # 若 include_model=True
        "布局1":  [polyA, polyB, ...],
        "布局sy": [ ... ],
        ...
      }
    """
    # 依赖 CAD_file_operations 的布局/空间切换函数（用户已在需求中给出参考实现）
    from scripts.CAD_file_operations import (
        set_space_mode,
        switch_to_layout,
        get_layout_names,
    )
    ...
    if include_model:
        try:
            set_space_mode(1)  # TILEMODE=1
        except Exception:
            pass

        model_polys = select_all_polylines_in_active_doc(only_current_space=True)
        model_rects = [p for p in model_polys if is_rectangular_polyline(p)]
        model_rects = remove_duplicate_polylines_dynamic(model_rects)
        model_areas = compute_print_areas_from_rectangles(
            model_rects,
            space_label="Model",
            delete_pseudo_outer=True,
            draw_outline_before_delete=True,
        )
        result["Model"] = model_areas

    # 2) 各布局（PaperSpace）
    layout_names = get_layout_names(exclude_model=exclude_model_tab)

    for lname in layout_names:
        try:
            ok = switch_to_layout(lname)
        except Exception:
            ok = False
        if not ok:
            continue
        ...
        try:
            set_space_mode(0)  # TILEMODE=0
        except Exception:
            pass

        paper_polys = select_all_polylines_in_active_doc(only_current_space=True)
        paper_rects = [p for p in paper_polys if is_rectangular_polyline(p)]
        paper_rects = remove_duplicate_polylines_dynamic(paper_rects)
        paper_areas = compute_print_areas_from_rectangles(
            paper_rects,
            space_label=f"Layout:{lname}",
            delete_pseudo_outer=True,
            draw_outline_before_delete=True,
        )
        result[lname] = paper_areas

    return result


def get_print_area_polylines() -> Dict[str, List[Any]]:
    """
    兼容入口：按布局名返回打印区域（模型空间也作为 "Model" 键）。
    """
    return get_print_areas_by_layout(include_model=True, exclude_model_tab=True)
