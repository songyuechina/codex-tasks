...
- 本目录中若出现“打印智能体”字样，当前统一按历史口径理解
- 当前更准确的概念是：打印执行工作区 / 打印执行链
- 文件名 `PRINT_AGENT_SPEC.md` 也属于历史命名保留

本目录是打印执行工作区。

目标不是只保存几个脚本，而是建立一套让后续项目主管或执行脚本能够：
...
3. `PRINT_DISPATCH_PROTOCOL.md`
   再看谁可以调度打印执行工作区、输入输出契约、默认接单判断。
4. `PRINT_AGENT_SPEC.md`
   再看打印执行链的角色定位、职责边界、能力要求。
5. `PRINT_WORKFLOW.md`
