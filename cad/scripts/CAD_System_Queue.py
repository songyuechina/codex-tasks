...
class MasterRunner:
    ...
    def setup_dashboard(self):
        ...
        # 2. 全自动化控制区域 (新增)
        frame_auto = ttk.LabelFrame(self.tab_dashboard, text="全自动化流水线 (One-Click Pipeline)", padding=10)
        frame_auto.pack(fill='x', padx=10, pady=10)
        
        # 第一排：单模块自动化
        f_auto1 = ttk.Frame(frame_auto)
        f_auto1.pack(fill='x', pady=5)
        f_auto1.columnconfigure(0, weight=1)
        f_auto1.columnconfigure(1, weight=1)
        f_auto1.columnconfigure(2, weight=1)
        
        ttk.Button(f_auto1, text="FUNC1: 一键插图签\n(识别+插入+映射+填写)", style='Dragon.TButton', 
                   command=lambda: self.run_macro("FUNC1")).grid(row=0, column=0, sticky='ew', padx=5)
        
        ttk.Button(f_auto1, text="FUNC2: 一键编目录\n(模板+图签+书写+合并)", style='Dragon.TButton', 
                   command=lambda: self.run_macro("FUNC2")).grid(row=0, column=1, sticky='ew', padx=5)

        ttk.Button(f_auto1, text="FUNC3: 一键打印\n(当前文件全套)", style='Dragon.TButton', 
                   command=lambda: self.run_macro("FUNC3")).grid(row=0, column=2, sticky='ew', padx=5)

        # 第二排：组合自动化
        f_auto2 = ttk.Frame(frame_auto)
        f_auto2.pack(fill='x', pady=10)
        f_auto2.columnconfigure(0, weight=1)
        f_auto2.columnconfigure(1, weight=1)
        f_auto2.columnconfigure(2, weight=1)

        ttk.Button(f_auto2, text="FUNC4: 插图签 + 编目录\n(FUNC1 + FUNC2)", style='Repair.TButton', 
                   command=lambda: self.run_macro("FUNC4")).grid(row=0, column=0, sticky='ew', padx=5)
                   
        ttk.Button(f_auto2, text="FUNC5: 编目录 + 打印\n(FUNC2 + FUNC3)", style='Repair.TButton', 
                   command=lambda: self.run_macro("FUNC5")).grid(row=0, column=1, sticky='ew', padx=5)

        ttk.Button(f_auto2, text="FUNC6: 一键跑完全程\n(FUNC1 + FUNC2 + FUNC3)", style='Data.TButton', 
                   command=lambda: self.run_macro("FUNC6")).grid(row=0, column=2, sticky='ew', padx=5)

        # 说明
        ttk.Label(self.tab_dashboard, text="\n⚠️ 注意：自动化基于【数据中心】的配置（操作目标/布局名/比例），运行前请确认配置无误。", 
                  font=('微软雅黑', 9, 'italic'), foreground='red').pack(anchor='w', padx=10)
...
'''
        ...
        sel_conf_in = self.dc_vars["select_config"].get().strip()
        sel_conf_arg = sel_conf_in if sel_conf_in.lower() != "none" else "None"
        if sel_conf_arg != "None" and sel_conf_arg.isdigit(): sel_conf_arg = int(sel_conf_arg)
        use_cache_arg = str(self.dc_vars["use_cache"].get())
        
        # --- 代码块定义 (为了复用) ---
        
        # 代码块: 图签全流程 (识别 -> 插入 -> 映射 -> 填写)
        CODE_FUNC1 = f'''
print("\\n🔹 [FUNC1] 开始: 一键插图签流水线...")
# 1. 识别
print("   >> 1.1 识别打印区域...")
dy = cb.smart_select_polylines(layout_name={layout_arg}, operate_target="{op_target}", select_config={sel_conf_arg}, use_cache={use_cache_arg}, verbose=False)
GLOBAL_CTX['dy'] = dy
if not dy:
    print("   ❌ 识别失败，流程终止")
else:
    ...
    print("   >> 1.2 插入图签...")
    lay = {layout_arg}
    if "{op_target}" == "Model" and not lay:
        cb.insert_and_scale_labels_area_power(dy, layername="dy_quyu", delpan=0, debug=False)
    elif "{op_target}" == "Layout":
        cb.insert_and_scale_labels_paper_space(dy, layername="dy_quyu", layout_name=lay, delpan=0, debug=False)
    else:
        cb.insert_and_scale_labels_area_power(dy, layername="dy_quyu", delpan=0, debug=False)
        cb.repair_sp_insert(target_layout_name=lay)
    ...
    # 3. 重建映射
    print("   >> 1.3 重建映射 (CTQ)...")
    ctq = cb.smart_rebuild_print_info(layout_name={layout_arg}, operate_target="{op_target}", select_config={sel_conf_arg}, use_cache=False, verbose=False)
    GLOBAL_CTX['ctq'] = ctq
    ...
    if ctq and len(ctq) >= 3:
        print("   >> 1.4 自动填写图名...")
        if "{op_target}" == "Layout":
            cb.process_drawing_names_and_fill_titleblocks(ctq, layout_target=lay, num1=14, num2=3, layername="DIM_SYMB", start_index=1, prefix=None)
        else:
            cb.process_drawing_names_and_fill_titleblocks(ctq, num1=14, num2=3, layername="DIM_SYMB", start_index=1, prefix=None)
        print("✅ [FUNC1] 图签处理完毕")
    else:
        print("❌ 映射建立失败，无法填写图名")
'''
...
mulu_n = {ctx['mulu_n']}
print("   >> 2.1 插入目录模板...")
if cb.bianmulu_func1(dwg_path=None, mulu_xuhao=mulu_n):
    print("   >> 2.2 插入目录图签...")
    if cb.bianmulu_func2(dwg_path=None):
        print("   >> 2.3 书写目录内容...")
        if cb.bianmulu_func3(dwg_path=None, mubanxuhao=mulu_n):
            print("   >> 2.4 合并到主文件...")
            if cb.bianmulu_func4(dwg_path=None):
                print("✅ [FUNC2] 目录编制完毕")
            else: print("❌ 合并失败")
        else: print("❌ 书写失败")
    else: print("❌ 插入目录图签失败")
...
if op == "Model" and not lay:
    print("   >> 模型空间按文件打印...")
    cb.print_dwg_file_model(file_path=None, ctb=ctb_file)
elif op == "Layout":
    print(f"   >> 布局空间打印 ({{lay}})...")
    cb.print_dwg_file_layout(file_path=None, layout_name=lay, ctb=ctb_file)
else:
    print("   >> 混合空间打印...")
    try: cb.print_dwg_file_model_h(file_path=None, ctb=ctb_file)
    except: pass
    try: cb.print_dwg_file_layout_h(file_path=None, layout_name=lay, ctb=ctb_file)
    except: pass
print("✅ [FUNC3] 打印指令已发送")
'''
        ...
        file_open_code = ""
        if target_file:
             file_open_code = f'try: cfo.open_file(os.path.join(r"{dwg_dir_safe}", "{target_file}")); except: pass'
...
'''
        ...
        if macro_name == "FUNC1":
            script = base_ctx + CODE_FUNC1
            self.send_script_to_idle(script)

        elif macro_name == "FUNC2":
            # FUNC2 需要目录模板检查
            if not ctx['cat_tpl']: 
                messagebox.showerror("Err", "请先在【编目录】Tab或总看板选择目录模板")
                return
            script = base_ctx + CODE_FUNC2
            self.send_script_to_idle(script)

        elif macro_name == "FUNC3":
            script = base_ctx + CODE_FUNC3
            self.send_script_to_idle(script)

        elif macro_name == "FUNC4":
            if not ctx['cat_tpl']: messagebox.showerror("Err", "请选择目录模板"); return
            script = base_ctx + CODE_FUNC1 + CODE_FUNC2
            self.send_script_to_idle(script)

        elif macro_name == "FUNC5":
            if not ctx['cat_tpl']: messagebox.showerror("Err", "请选择目录模板"); return
            script = base_ctx + CODE_FUNC2 + CODE_FUNC3
            self.send_script_to_idle(script)

        elif macro_name == "FUNC6":
            if not ctx['cat_tpl']: messagebox.showerror("Err", "请选择目录模板"); return
            script = base_ctx + CODE_FUNC1 + CODE_FUNC2 + CODE_FUNC3
            self.send_script_to_idle(script)
