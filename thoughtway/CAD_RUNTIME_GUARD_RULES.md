...
3. 本次真实执行是否已同时运行 `D:/codex-tasks/cad/system/cad_runtime_guard.py`
4. 本次测试或回归所用 DWG 是否统一放在 `D:/codex-tasks/cad/tests/`
...
对整个项目的一般 DWG 任务，当前默认纪律是：

1. 先挂载守护脚本组，其中必须包含 `cad_runtime_guard.py`
2. 再走受控入口
3. 再打开测试文件或目标文件

也就是说，“真实运行时默认并行 runtime guard”不再只是打印子系统规则，而是项目级规则。
...
4. 再生成打印计划
5. 再进入真实打印
6. 每完成一个 DWG 后，先做收尾关图，再调用 `CAD_core.cad_zt_oneb()` 把 CAD 环境压回 `1 个进程 + 1 个空白天正会话`

## 7.1 对所有 CAD 测试任务的补充要求

不仅打印任务，普通块处理、图签处理、目录处理、文字处理等真实 CAD 测试任务也应遵守：

1. 测试 DWG 统一放在 `D:/codex-tasks/cad/tests/`
2. 真实测试前默认并行运行 `cad_runtime_guard.py`
3. 若测试会改写原文件，先在 `cad/tests/备份/` 中保留备份

---

## 8. 面向任务下达者的建议

以后给项目总管或打印执行工作区下任务时，推荐把任务开头固定成这种形式：
